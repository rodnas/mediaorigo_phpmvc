...
// Page object
$CoreLanguageEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { core_language: currentTable } });
var currentForm, currentPageID;
var fcore_languageedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fcore_languageedit = new ew.Form("fcore_languageedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fcore_languageedit;

    // Add fields
    var fields = currentTable.fields;
    fcore_languageedit.addFields([
    ...
    // Form_CustomValidate
    fcore_languageedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcore_languageedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fcore_languageedit.lists.core_statusID = <?= $Page->core_statusID->toClientList($Page) ?>;
    fcore_languageedit.lists.modifyUserID = <?= $Page->modifyUserID->toClientList($Page) ?>;
    loadjs.done("fcore_languageedit");
});
</script>
<script>
...
<?php } ?>
<form name="fcore_languageedit" id="fcore_languageedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } else { ?>
<table id="tbl_core_languageedit" class="table table-striped table-bordered table-hover table-sm ew-desktop-table"><!-- table* -->
<?php } ?>
...
<input type="hidden" data-table="core_language" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } else { ?>
...
<input type="hidden" name="fa_x_imageURL" id= "fa_x_imageURL" value="<?= (Post("fa_x_imageURL") == "0") ? "0" : "1" ?>">
...
<span id="el_core_language_core_statusID">
    <select
        ...
        data-select2-id="fcore_languageedit_x_core_statusID"
...
<script>
loadjs.ready("fcore_languageedit", function() {
    var options = { name: "x_core_statusID", selectId: "fcore_languageedit_x_core_statusID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcore_languageedit.lists.core_statusID.lookupOptions.length) {
        options.data = { id: "x_core_statusID", form: "fcore_languageedit" };
    } else {
        options.ajax = { id: "x_core_statusID", form: "fcore_languageedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
