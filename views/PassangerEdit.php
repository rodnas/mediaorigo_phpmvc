...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpassangeredit = new ew.Form("fpassangeredit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fpassangeredit;

    // Add fields
    var fields = currentTable.fields;
    fpassangeredit.addFields([
    ...
    // Form_CustomValidate
    fpassangeredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpassangeredit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fpassangeredit.lists.modifyUserID = <?= $Page->modifyUserID->toClientList($Page) ?>;
    fpassangeredit.lists.core_statusID = <?= $Page->core_statusID->toClientList($Page) ?>;
    fpassangeredit.lists.core_languageID = <?= $Page->core_languageID->toClientList($Page) ?>;
    loadjs.done("fpassangeredit");
});
</script>
<script>
...
<?php } ?>
<form name="fpassangeredit" id="fpassangeredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } else { ?>
<table id="tbl_passangeredit" class="table table-striped table-bordered table-hover table-sm ew-desktop-table"><!-- table* -->
<?php } ?>
<?php if ($Page->id->Visible) { // id ?>
<?php if ($Page->IsMobileOrModal) { ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_passanger_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_passanger_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="passanger" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } else { ?>
    <tr id="r_id"<?= $Page->id->rowAttributes() ?>>
        <td class="<?= $Page->TableLeftColumnClass ?>"><span id="elh_passanger_id"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></span></td>
...
<script>
loadjs.ready(["fpassangeredit", "datetimepicker"], function () {
    let format = "<?= "y-MM-dd" ?>",
        options = {
            localization: {
    ...
    ew.createDateTimePicker("fpassangeredit", "x_transport_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_passanger_core_statusID">
    <select
        ...
        data-select2-id="fpassangeredit_x_core_statusID"
...
<script>
loadjs.ready("fpassangeredit", function() {
    var options = { name: "x_core_statusID", selectId: "fpassangeredit_x_core_statusID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpassangeredit.lists.core_statusID.lookupOptions.length) {
        options.data = { id: "x_core_statusID", form: "fpassangeredit" };
    } else {
        options.ajax = { id: "x_core_statusID", form: "fpassangeredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_passanger_core_languageID">
    <select
        ...
        data-select2-id="fpassangeredit_x_core_languageID"
        data-table="passanger"
...
<script>
loadjs.ready("fpassangeredit", function() {
    var options = { name: "x_core_languageID", selectId: "fpassangeredit_x_core_languageID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpassangeredit.lists.core_languageID.lookupOptions.length) {
        options.data = { id: "x_core_languageID", form: "fpassangeredit" };
    } else {
        options.ajax = { id: "x_core_languageID", form: "fpassangeredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
