...
// Page object
$VehicleTypeEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { vehicle_type: currentTable } });
var currentForm, currentPageID;
var fvehicle_typeedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fvehicle_typeedit = new ew.Form("fvehicle_typeedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fvehicle_typeedit;

    // Add fields
    var fields = currentTable.fields;
    fvehicle_typeedit.addFields([
    ...
    // Form_CustomValidate
    fvehicle_typeedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fvehicle_typeedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fvehicle_typeedit.lists.modifyUserID = <?= $Page->modifyUserID->toClientList($Page) ?>;
    fvehicle_typeedit.lists.core_statusID = <?= $Page->core_statusID->toClientList($Page) ?>;
    loadjs.done("fvehicle_typeedit");
});
</script>
<script>
...
<?php } ?>
<form name="fvehicle_typeedit" id="fvehicle_typeedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } else { ?>
<table id="tbl_vehicle_typeedit" class="table table-striped table-bordered table-hover table-sm ew-desktop-table"><!-- table* -->
<?php } ?>
<?php if ($Page->id->Visible) { // id ?>
<?php if ($Page->IsMobileOrModal) { ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_vehicle_type_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_vehicle_type_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="vehicle_type" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } else { ?>
    <tr id="r_id"<?= $Page->id->rowAttributes() ?>>
        <td class="<?= $Page->TableLeftColumnClass ?>"><span id="elh_vehicle_type_id"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></span></td>
...
<span id="el_vehicle_type_core_statusID">
    <select
        ...
        data-select2-id="fvehicle_typeedit_x_core_statusID"
...
<script>
loadjs.ready("fvehicle_typeedit", function() {
    var options = { name: "x_core_statusID", selectId: "fvehicle_typeedit_x_core_statusID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fvehicle_typeedit.lists.core_statusID.lookupOptions.length) {
        options.data = { id: "x_core_statusID", form: "fvehicle_typeedit" };
    } else {
        options.ajax = { id: "x_core_statusID", form: "fvehicle_typeedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
