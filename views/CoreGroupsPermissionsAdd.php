...
// Page object
$CoreGroupsPermissionsAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { core_groups_permissions: currentTable } });
var currentForm, currentPageID;
var fcore_groups_permissionsadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fcore_groups_permissionsadd = new ew.Form("fcore_groups_permissionsadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fcore_groups_permissionsadd;

    // Add fields
    var fields = currentTable.fields;
    fcore_groups_permissionsadd.addFields([
    ...
    // Form_CustomValidate
    fcore_groups_permissionsadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcore_groups_permissionsadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fcore_groups_permissionsadd.lists.core_groupsID = <?= $Page->core_groupsID->toClientList($Page) ?>;
    loadjs.done("fcore_groups_permissionsadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fcore_groups_permissionsadd" id="fcore_groups_permissionsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } else { ?>
<table id="tbl_core_groups_permissionsadd" class="table table-striped table-bordered table-hover table-sm ew-desktop-table"><!-- table* -->
<?php } ?>
...
<span id="el_core_groups_permissions_core_groupsID">
    <select
        ...
        data-select2-id="fcore_groups_permissionsadd_x_core_groupsID"
...
<script>
loadjs.ready("fcore_groups_permissionsadd", function() {
    var options = { name: "x_core_groupsID", selectId: "fcore_groups_permissionsadd_x_core_groupsID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcore_groups_permissionsadd.lists.core_groupsID.lookupOptions.length) {
        options.data = { id: "x_core_groupsID", form: "fcore_groups_permissionsadd" };
    } else {
        options.ajax = { id: "x_core_groupsID", form: "fcore_groups_permissionsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
