...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fcore_usersadd = new ew.Form("fcore_usersadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fcore_usersadd;

    // Add fields
    var fields = currentTable.fields;
    fcore_usersadd.addFields([
    ...
    // Form_CustomValidate
    fcore_usersadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcore_usersadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fcore_usersadd.lists.core_languageID = <?= $Page->core_languageID->toClientList($Page) ?>;
    fcore_usersadd.lists.core_statusID = <?= $Page->core_statusID->toClientList($Page) ?>;
    fcore_usersadd.lists.core_groupsID = <?= $Page->core_groupsID->toClientList($Page) ?>;
    loadjs.done("fcore_usersadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fcore_usersadd" id="fcore_usersadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } else { ?>
<table id="tbl_core_usersadd" class="table table-striped table-bordered table-hover table-sm ew-desktop-table"><!-- table* -->
<?php } ?>
...
<div class="input-group" id="ig__password">
    <input type="password" autocomplete="new-password" data-table="core_users" data-field="x__password" name="x__password" id="x__password" size="30" maxlength="40" placeholder="<?= HtmlEncode($Page->_password->getPlaceHolder()) ?>"<?= $Page->_password->editAttributes() ?> aria-describedby="x__password_help">
...
<span id="el_core_users_core_languageID">
    <select
        ...
        data-select2-id="fcore_usersadd_x_core_languageID"
...
<script>
loadjs.ready("fcore_usersadd", function() {
    var options = { name: "x_core_languageID", selectId: "fcore_usersadd_x_core_languageID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcore_usersadd.lists.core_languageID.lookupOptions.length) {
        options.data = { id: "x_core_languageID", form: "fcore_usersadd" };
    } else {
        options.ajax = { id: "x_core_languageID", form: "fcore_usersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_core_users_core_statusID">
    <select
        ...
        data-select2-id="fcore_usersadd_x_core_statusID"
        data-table="core_users"
...
<script>
loadjs.ready("fcore_usersadd", function() {
    var options = { name: "x_core_statusID", selectId: "fcore_usersadd_x_core_statusID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcore_usersadd.lists.core_statusID.lookupOptions.length) {
        options.data = { id: "x_core_statusID", form: "fcore_usersadd" };
    } else {
        options.ajax = { id: "x_core_statusID", form: "fcore_usersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_core_users_core_groupsID">
    <select
        ...
        data-select2-id="fcore_usersadd_x_core_groupsID"
        data-table="core_users"
...
<script>
loadjs.ready("fcore_usersadd", function() {
    var options = { name: "x_core_groupsID", selectId: "fcore_usersadd_x_core_groupsID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcore_usersadd.lists.core_groupsID.lookupOptions.length) {
        options.data = { id: "x_core_groupsID", form: "fcore_usersadd" };
    } else {
        options.ajax = { id: "x_core_groupsID", form: "fcore_usersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
