...
var fcore_statusedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fcore_statusedit = new ew.Form("fcore_statusedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fcore_statusedit;

    // Add fields
    var fields = currentTable.fields;
    fcore_statusedit.addFields([
    ...
    // Form_CustomValidate
    fcore_statusedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcore_statusedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fcore_statusedit.lists.core_languageID = <?= $Page->core_languageID->toClientList($Page) ?>;
    fcore_statusedit.lists.core_statusID = <?= $Page->core_statusID->toClientList($Page) ?>;
    fcore_statusedit.lists.modifyUserID = <?= $Page->modifyUserID->toClientList($Page) ?>;
    loadjs.done("fcore_statusedit");
});
</script>
<script>
...
<?php } ?>
<form name="fcore_statusedit" id="fcore_statusedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } else { ?>
<table id="tbl_core_statusedit" class="table table-striped table-bordered table-hover table-sm ew-desktop-table"><!-- table* -->
<?php } ?>
...
<span id="el_core_status_core_languageID">
    <select
        ...
        data-select2-id="fcore_statusedit_x_core_languageID"
...
<script>
loadjs.ready("fcore_statusedit", function() {
    var options = { name: "x_core_languageID", selectId: "fcore_statusedit_x_core_languageID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcore_statusedit.lists.core_languageID.lookupOptions.length) {
        options.data = { id: "x_core_languageID", form: "fcore_statusedit" };
    } else {
        options.ajax = { id: "x_core_languageID", form: "fcore_statusedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_core_status_core_statusID">
    <select
        ...
        data-select2-id="fcore_statusedit_x_core_statusID"
        data-table="core_status"
...
<script>
loadjs.ready("fcore_statusedit", function() {
    var options = { name: "x_core_statusID", selectId: "fcore_statusedit_x_core_statusID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcore_statusedit.lists.core_statusID.lookupOptions.length) {
        options.data = { id: "x_core_statusID", form: "fcore_statusedit" };
    } else {
        options.ajax = { id: "x_core_statusID", form: "fcore_statusedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
    <input type="hidden" data-table="core_status" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
