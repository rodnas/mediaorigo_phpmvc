...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fcargoadd = new ew.Form("fcargoadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fcargoadd;
    ...
    // Form_CustomValidate
    fcargoadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcargoadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fcargoadd.lists.insertUserID = <?= $Page->insertUserID->toClientList($Page) ?>;
    fcargoadd.lists.core_statusID = <?= $Page->core_statusID->toClientList($Page) ?>;
    fcargoadd.lists.core_languageID = <?= $Page->core_languageID->toClientList($Page) ?>;
    loadjs.done("fcargoadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fcargoadd" id="fcargoadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } else { ?>
<table id="tbl_cargoadd" class="table table-striped table-bordered table-hover table-sm ew-desktop-table"><!-- table* -->
<?php } ?>
...
<script>
loadjs.ready(["fcargoadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcargoadd", "x_transport_date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_cargo_core_statusID">
    <select
        ...
        data-select2-id="fcargoadd_x_core_statusID"
        data-table="cargo"
...
loadjs.ready("fcargoadd", function() {
    var options = { name: "x_core_statusID", selectId: "fcargoadd_x_core_statusID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcargoadd.lists.core_statusID.lookupOptions.length) {
        options.data = { id: "x_core_statusID", form: "fcargoadd" };
    } else {
        options.ajax = { id: "x_core_statusID", form: "fcargoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_cargo_core_languageID">
    <select
        ...
        data-select2-id="fcargoadd_x_core_languageID"
        data-table="cargo"
...
<script>
loadjs.ready("fcargoadd", function() {
    var options = { name: "x_core_languageID", selectId: "fcargoadd_x_core_languageID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcargoadd.lists.core_languageID.lookupOptions.length) {
        options.data = { id: "x_core_languageID", form: "fcargoadd" };
    } else {
        options.ajax = { id: "x_core_languageID", form: "fcargoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
