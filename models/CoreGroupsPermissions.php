...
/**
 * Table class for core_groups_permissions
 */
class CoreGroupsPermissions extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'core_groups_permissions';
        $this->TableName = 'core_groups_permissions';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`core_groups_permissions`";
        $this->Dbid = 'DB';
        ...
        $this->core_groupsID->IsPrimaryKey = true; // Primary key field
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`core_groups_permissions`";
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('core_groupsID', $rs)) {
                AddFilter($where, QuotedName('core_groupsID', $this->Dbid) . '=' . QuotedValue($rs['core_groupsID'], $this->core_groupsID->DataType, $this->Dbid));
            }
    ...
    {
        return "`core_groupsID` = @core_groupsID@ AND `tablename` = '@_tablename@'";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->core_groupsID->CurrentValue : $this->core_groupsID->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 2) {
            if ($current) {
                $this->core_groupsID->CurrentValue = $keys[0];
            } else {
                $this->core_groupsID->OldValue = $keys[0];
            }
            if ($current) {
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('core_groupsID', $row) ? $row['core_groupsID'] : null;
        } else {
            $val = $this->core_groupsID->OldValue !== null ? $this->core_groupsID->OldValue : $this->core_groupsID->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@core_groupsID@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        if (is_array($row)) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("CoreGroupsPermissionsList");
    }
    ...
    {
        global $Language;
        if ($pageName == "CoreGroupsPermissionsView") {
            return $Language->phrase("View");
        } elseif ($pageName == "CoreGroupsPermissionsEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "CoreGroupsPermissionsAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "CoreGroupsPermissionsView";
            case Config("API_ADD_ACTION"):
                return "CoreGroupsPermissionsAdd";
            case Config("API_EDIT_ACTION"):
                return "CoreGroupsPermissionsEdit";
            case Config("API_DELETE_ACTION"):
                return "CoreGroupsPermissionsDelete";
            case Config("API_LIST_ACTION"):
                return "CoreGroupsPermissionsList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("CoreGroupsPermissionsView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("CoreGroupsPermissionsView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "CoreGroupsPermissionsAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "CoreGroupsPermissionsAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CoreGroupsPermissionsEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CoreGroupsPermissionsAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("CoreGroupsPermissionsDelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "\"core_groupsID\":" . JsonEncode($this->core_groupsID->CurrentValue, "number");
    ...
    {
        if ($this->core_groupsID->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->core_groupsID->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("core_groupsID") ?? Route("core_groupsID")) !== null) {
                $arKey[] = $keyValue;
        ...
        if (is_array($arKeys)) {
            foreach ($arKeys as $key) {
                ...
                if (!is_numeric($key[0])) { // core_groupsID
                    continue;
                }
                $ar[] = $key;
            }
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->core_groupsID->CurrentValue = $key[0];
            } else {
                $this->core_groupsID->OldValue = $key[0];
            }
            if ($setCurrent) {
