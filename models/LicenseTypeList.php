 ...
 * Page class
 */
class LicenseTypeList extends LicenseType
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LicenseTypeList";
    ...
    public $FormName = "flicense_typelist";
    ...
    public function __construct()
    {
        ...
        // Table object (license_type)
        if (!isset($GLOBALS["license_type"]) || get_class($GLOBALS["license_type"]) == PROJECT_NAMESPACE . "license_type") {
            $GLOBALS["license_type"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "LicenseTypeAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "LicenseTypeDelete";
        $this->MultiUpdateUrl = "LicenseTypeUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'license_type');
        }
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->modifyWhen);
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "flicense_typesrch", $filters);
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"flicense_typelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"flicense_typelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"flicense_typesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"flicense_typesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="flicense_typelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->modifyUserID->ViewValue = FormatNumber($this->modifyUserID->ViewValue, $this->modifyUserID->formatPattern());
            $this->modifyUserID->ViewCustomAttributes = "";

            // modifyWhen
            $curVal = strval($this->modifyWhen->CurrentValue);
            if ($curVal != "") {
                $this->modifyWhen->ViewValue = $this->modifyWhen->lookupCacheOption($curVal);
                if ($this->modifyWhen->ViewValue === null) { // Lookup from database
                    $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                    $sqlWrk = $this->modifyWhen->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCacheImpl($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                    $ari = count($rswrk);
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->modifyWhen->Lookup->renderViewRow($rswrk[0]);
                        $this->modifyWhen->ViewValue = $this->modifyWhen->displayValue($arwrk);
                    } else {
                        $this->modifyWhen->ViewValue = FormatDateTime($this->modifyWhen->CurrentValue, $this->modifyWhen->formatPattern());
                    }
                }
            } else {
                $this->modifyWhen->ViewValue = null;
            }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flicense_typesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"LicenseTypeSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        $item->Visible = true;
