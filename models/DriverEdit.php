 ...
 * Page class
 */
class DriverEdit extends Driver
{
    use MessagesTrait;
    ...
    public function run()
    {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("DriverList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("DriverList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "DriverList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'birth_year' first before field var 'x_birth_year'
        $val = $CurrentForm->hasValue("birth_year") ? $CurrentForm->getValue("birth_year") : $CurrentForm->getValue("x_birth_year");
        if (!$this->birth_year->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->birth_year->Visible = false; // Disable update for API request
            } else {
                $this->birth_year->setFormValue($val, true, $validate);
            }
        }

        // Check field name 'license_typeID' first before field var 'x_license_typeID'
        $val = $CurrentForm->hasValue("license_typeID") ? $CurrentForm->getValue("license_typeID") : $CurrentForm->getValue("x_license_typeID");
        if (!$this->license_typeID->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->license_typeID->Visible = false; // Disable update for API request
            } else {
                $this->license_typeID->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->birth_year->CurrentValue = $this->birth_year->FormValue;
        $this->license_typeID->CurrentValue = $this->license_typeID->FormValue;
    ...
    public function renderRow()
    {
        ...
        // name
        $this->name->RowCssClass = $this->IsMobileOrModal ? "row" : "";

        // birth_year
        $this->birth_year->RowCssClass = $this->IsMobileOrModal ? "row" : "";

        // license_typeID
        $this->license_typeID->RowCssClass = $this->IsMobileOrModal ? "row" : "";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->birth_year->EditValue = HtmlEncode($this->birth_year->CurrentValue);
            $this->birth_year->PlaceHolder = RemoveHtml($this->birth_year->caption());
            if (strval($this->birth_year->EditValue) != "" && is_numeric($this->birth_year->EditValue)) {
                $this->birth_year->EditValue = $this->birth_year->EditValue;
            }

            // license_typeID
            $this->license_typeID->setupEditAttributes();
            $this->license_typeID->EditCustomAttributes = "";
            $curVal = trim(strval($this->license_typeID->CurrentValue));
            if ($curVal != "") {
                $this->license_typeID->ViewValue = $this->license_typeID->lookupCacheOption($curVal);
            } else {
                $this->license_typeID->ViewValue = $this->license_typeID->Lookup !== null && is_array($this->license_typeID->lookupOptions()) ? $curVal : null;
            }
            if ($this->license_typeID->ViewValue !== null) { // Load from cache
                $this->license_typeID->EditValue = array_values($this->license_typeID->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", $this->license_typeID->CurrentValue, DATATYPE_NUMBER, "");
                }
    ...
    protected function validateForm()
    {
        ...
        if ($this->birth_year->Required) {
            if (!$this->birth_year->IsDetailKey && EmptyValue($this->birth_year->FormValue)) {
                $this->birth_year->addErrorMessage(str_replace("%s", $this->birth_year->caption(), $this->birth_year->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->birth_year->FormValue)) {
            $this->birth_year->addErrorMessage($this->birth_year->getErrorMessage(false));
        }
        if ($this->license_typeID->Required) {
            if (!$this->license_typeID->IsDetailKey && EmptyValue($this->license_typeID->FormValue)) {
                $this->license_typeID->addErrorMessage(str_replace("%s", $this->license_typeID->caption(), $this->license_typeID->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // birth_year
        $this->birth_year->setDbValueDef($rsnew, $this->birth_year->CurrentValue, null, $this->birth_year->ReadOnly);

        // license_typeID
        $this->license_typeID->setDbValueDef($rsnew, $this->license_typeID->CurrentValue, 0, $this->license_typeID->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("DriverList"), "", $this->TableVar, true);
        $pageId = "edit";
