...
class CargoList extends Cargo
{
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->place->AdvancedSearch->toJson(), ","); // Field place
        $filterList = Concat($filterList, $this->cargo_weight->AdvancedSearch->toJson(), ","); // Field cargo_weight
        $filterList = Concat($filterList, $this->transport_date->AdvancedSearch->toJson(), ","); // Field transport_date
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcargosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field place
        $this->place->AdvancedSearch->SearchValue = @$filter["x_place"];
        $this->place->AdvancedSearch->SearchOperator = @$filter["z_place"];
        $this->place->AdvancedSearch->SearchCondition = @$filter["v_place"];
        $this->place->AdvancedSearch->SearchValue2 = @$filter["y_place"];
        $this->place->AdvancedSearch->SearchOperator2 = @$filter["w_place"];
        $this->place->AdvancedSearch->save();

        // Field cargo_weight
        $this->cargo_weight->AdvancedSearch->SearchValue = @$filter["x_cargo_weight"];
        $this->cargo_weight->AdvancedSearch->SearchOperator = @$filter["z_cargo_weight"];
        $this->cargo_weight->AdvancedSearch->SearchCondition = @$filter["v_cargo_weight"];
        $this->cargo_weight->AdvancedSearch->SearchValue2 = @$filter["y_cargo_weight"];
        $this->cargo_weight->AdvancedSearch->SearchOperator2 = @$filter["w_cargo_weight"];
        $this->cargo_weight->AdvancedSearch->save();

        // Field transport_date
        $this->transport_date->AdvancedSearch->SearchValue = @$filter["x_transport_date"];
        $this->transport_date->AdvancedSearch->SearchOperator = @$filter["z_transport_date"];
        $this->transport_date->AdvancedSearch->SearchCondition = @$filter["v_transport_date"];
        $this->transport_date->AdvancedSearch->SearchValue2 = @$filter["y_transport_date"];
        $this->transport_date->AdvancedSearch->SearchOperator2 = @$filter["w_transport_date"];
        $this->transport_date->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->place, $default, false); // place
        $this->buildSearchSql($where, $this->cargo_weight, $default, false); // cargo_weight
        $this->buildSearchSql($where, $this->transport_date, $default, false); // transport_date
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->place->AdvancedSearch->save(); // place
            $this->cargo_weight->AdvancedSearch->save(); // cargo_weight
            $this->transport_date->AdvancedSearch->save(); // transport_date
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        $searchFlds = [];
        $searchFlds[] = &$this->id;
        $searchFlds[] = &$this->place;
        $searchFlds[] = &$this->cargo_weight;
        $searchFlds[] = &$this->transport_date;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->place->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->cargo_weight->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->transport_date->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->place->AdvancedSearch->unsetSession();
        $this->cargo_weight->AdvancedSearch->unsetSession();
        $this->transport_date->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->place->AdvancedSearch->load();
        $this->cargo_weight->AdvancedSearch->load();
        $this->transport_date->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->place); // place
            $this->updateSort($this->cargo_weight); // cargo_weight
            $this->updateSort($this->transport_date); // transport_date
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id->setSort("");
                $this->place->setSort("");
                $this->cargo_weight->setSort("");
                $this->transport_date->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcargolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcargolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("place", $this->createColumnOption("place"));
            $option->add("cargo_weight", $this->createColumnOption("cargo_weight"));
            $option->add("transport_date", $this->createColumnOption("transport_date"));
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcargolist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // place
        if ($this->place->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->place->AdvancedSearch->SearchValue != "" || $this->place->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // cargo_weight
        if ($this->cargo_weight->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cargo_weight->AdvancedSearch->SearchValue != "" || $this->cargo_weight->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // transport_date
        if ($this->transport_date->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->transport_date->AdvancedSearch->SearchValue != "" || $this->transport_date->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (!$this->place->Raw) {
                $this->place->AdvancedSearch->SearchValue = HtmlDecode($this->place->AdvancedSearch->SearchValue);
            }
            $this->place->EditValue = HtmlEncode($this->place->AdvancedSearch->SearchValue);
            ...
            $this->cargo_weight->EditValue = HtmlEncode($this->cargo_weight->AdvancedSearch->SearchValue);
            ...
            $this->transport_date->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->transport_date->AdvancedSearch->SearchValue, $this->transport_date->formatPattern()), $this->transport_date->formatPattern()));
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->cargo_weight->AdvancedSearch->SearchValue)) {
            $this->cargo_weight->addErrorMessage($this->cargo_weight->getErrorMessage(false));
        }
        if (!CheckDate($this->transport_date->AdvancedSearch->SearchValue, $this->transport_date->formatPattern())) {
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"CargoSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        $item->Visible = true;
