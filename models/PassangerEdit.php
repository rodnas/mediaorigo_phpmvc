 ...
 * Page class
 */
class PassangerEdit extends Passanger
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'passanger';

    // Page object name
    public $PageObjName = "PassangerEdit";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["passanger"]) || get_class($GLOBALS["passanger"]) == PROJECT_NAMESPACE . "passanger") {
            $GLOBALS["passanger"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'passanger');
        }
    ...
    public function run()
    {
        ...
        $this->how_many_people->setVisibility();
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("PassangerList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("PassangerList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "PassangerList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'how_many_people' first before field var 'x_how_many_people'
        $val = $CurrentForm->hasValue("how_many_people") ? $CurrentForm->getValue("how_many_people") : $CurrentForm->getValue("x_how_many_people");
        if (!$this->how_many_people->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->how_many_people->Visible = false; // Disable update for API request
            } else {
                $this->how_many_people->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->how_many_people->CurrentValue = $this->how_many_people->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['customer_name'] = $this->customer_name->DefaultValue;
        $row['how_many_people'] = $this->how_many_people->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        // customer_name
        $this->customer_name->RowCssClass = $this->IsMobileOrModal ? "row" : "";

        // how_many_people
        $this->how_many_people->RowCssClass = $this->IsMobileOrModal ? "row" : "";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->how_many_people->EditValue = HtmlEncode($this->how_many_people->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->how_many_people->Required) {
            if (!$this->how_many_people->IsDetailKey && EmptyValue($this->how_many_people->FormValue)) {
                $this->how_many_people->addErrorMessage(str_replace("%s", $this->how_many_people->caption(), $this->how_many_people->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->how_many_people->FormValue)) {
            $this->how_many_people->addErrorMessage($this->how_many_people->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        // how_many_people
        $this->how_many_people->setDbValueDef($rsnew, $this->how_many_people->CurrentValue, null, $this->how_many_people->ReadOnly);

        // transport_date
        $this->transport_date->setDbValueDef($rsnew, UnFormatDateTime($this->transport_date->CurrentValue, $this->transport_date->formatPattern()), null, $this->transport_date->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PassangerList"), "", $this->TableVar, true);
        $pageId = "edit";
