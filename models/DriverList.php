 ...
 * Page class
 */
class DriverList extends Driver
{
    use MessagesTrait;
    ...
    public function __construct()
    {
        ...
        // Table object (driver)
        if (!isset($GLOBALS["driver"]) || get_class($GLOBALS["driver"]) == PROJECT_NAMESPACE . "driver") {
            $GLOBALS["driver"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "DriverAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "DriverDelete";
        $this->MultiUpdateUrl = "DriverUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'driver');
        }
    ...
    public function run()
    {
        ...
        $this->birth_year->setVisibility();
        $this->license_typeID->setVisibility();
        $this->insertUserID->Visible = false;
        $this->insertWhen->Visible = false;
        $this->modifyUserID->Visible = false;
        $this->modifyWhen->Visible = false;
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->license_typeID);
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->birth_year->AdvancedSearch->toJson(), ","); // Field birth_year
        $filterList = Concat($filterList, $this->license_typeID->AdvancedSearch->toJson(), ","); // Field license_typeID
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fdriversrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field birth_year
        $this->birth_year->AdvancedSearch->SearchValue = @$filter["x_birth_year"];
        $this->birth_year->AdvancedSearch->SearchOperator = @$filter["z_birth_year"];
        $this->birth_year->AdvancedSearch->SearchCondition = @$filter["v_birth_year"];
        $this->birth_year->AdvancedSearch->SearchValue2 = @$filter["y_birth_year"];
        $this->birth_year->AdvancedSearch->SearchOperator2 = @$filter["w_birth_year"];
        $this->birth_year->AdvancedSearch->save();

        // Field license_typeID
        $this->license_typeID->AdvancedSearch->SearchValue = @$filter["x_license_typeID"];
        $this->license_typeID->AdvancedSearch->SearchOperator = @$filter["z_license_typeID"];
        $this->license_typeID->AdvancedSearch->SearchCondition = @$filter["v_license_typeID"];
        $this->license_typeID->AdvancedSearch->SearchValue2 = @$filter["y_license_typeID"];
        $this->license_typeID->AdvancedSearch->SearchOperator2 = @$filter["w_license_typeID"];
        $this->license_typeID->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->birth_year, $default, false); // birth_year
        $this->buildSearchSql($where, $this->license_typeID, $default, false); // license_typeID
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->birth_year->AdvancedSearch->save(); // birth_year
            $this->license_typeID->AdvancedSearch->save(); // license_typeID
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->birth_year;
        $searchFlds[] = &$this->license_typeID;
        $searchFlds[] = &$this->insertUserID;
        $searchFlds[] = &$this->insertWhen;
        $searchFlds[] = &$this->modifyUserID;
        $searchFlds[] = &$this->modifyWhen;
        $searchFlds[] = &$this->core_statusID;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->birth_year->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->license_typeID->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->birth_year->AdvancedSearch->unsetSession();
        $this->license_typeID->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->birth_year->AdvancedSearch->load();
        $this->license_typeID->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->birth_year); // birth_year
            $this->updateSort($this->license_typeID); // license_typeID
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id->setSort("");
                $this->name->setSort("");
                $this->birth_year->setSort("");
                $this->license_typeID->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fdriverlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fdriverlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("birth_year", $this->createColumnOption("birth_year"));
            $option->add("license_typeID", $this->createColumnOption("license_typeID"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdriversrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdriversrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fdriverlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // birth_year
        if ($this->birth_year->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->birth_year->AdvancedSearch->SearchValue != "" || $this->birth_year->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // license_typeID
        if ($this->license_typeID->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->license_typeID->AdvancedSearch->SearchValue != "" || $this->license_typeID->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->birth_year->setDbValue($row['birth_year']);
        $this->license_typeID->setDbValue($row['license_typeID']);
    ...
    protected function newRow()
    {
        ...
        $row['birth_year'] = $this->birth_year->DefaultValue;
        $row['license_typeID'] = $this->license_typeID->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // birth_year
            $this->birth_year->ViewValue = $this->birth_year->CurrentValue;
            $this->birth_year->ViewCustomAttributes = "";

            // license_typeID
            $curVal = strval($this->license_typeID->CurrentValue);
            if ($curVal != "") {
                $this->license_typeID->ViewValue = $this->license_typeID->lookupCacheOption($curVal);
                if ($this->license_typeID->ViewValue === null) { // Lookup from database
                    $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                    $sqlWrk = $this->license_typeID->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->license_typeID->Lookup->renderViewRow($rswrk[0]);
                        $this->license_typeID->ViewValue = $this->license_typeID->displayValue($arwrk);
                    } else {
                        $this->license_typeID->ViewValue = FormatNumber($this->license_typeID->CurrentValue, $this->license_typeID->formatPattern());
                    }
                }
            } else {
                $this->license_typeID->ViewValue = null;
            }
            $this->license_typeID->ViewCustomAttributes = "";
            ...
            // birth_year
            $this->birth_year->LinkCustomAttributes = "";
            $this->birth_year->HrefValue = "";
            $this->birth_year->TooltipValue = "";

            // license_typeID
            $this->license_typeID->LinkCustomAttributes = "";
            $this->license_typeID->HrefValue = "";
            $this->license_typeID->TooltipValue = "";
            ...
            // birth_year
            $this->birth_year->setupEditAttributes();
            $this->birth_year->EditCustomAttributes = "";
            $this->birth_year->EditValue = HtmlEncode($this->birth_year->AdvancedSearch->SearchValue);
            $this->birth_year->PlaceHolder = RemoveHtml($this->birth_year->caption());

            // license_typeID
            $this->license_typeID->setupEditAttributes();
            $this->license_typeID->EditCustomAttributes = "";
            $curVal = trim(strval($this->license_typeID->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->license_typeID->AdvancedSearch->ViewValue = $this->license_typeID->lookupCacheOption($curVal);
            } else {
                $this->license_typeID->AdvancedSearch->ViewValue = $this->license_typeID->Lookup !== null && is_array($this->license_typeID->lookupOptions()) ? $curVal : null;
            }
            if ($this->license_typeID->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->license_typeID->EditValue = array_values($this->license_typeID->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", $this->license_typeID->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->license_typeID->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCacheImpl($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->license_typeID->EditValue = $arwrk;
            }
            $this->license_typeID->PlaceHolder = RemoveHtml($this->license_typeID->caption());
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->birth_year->AdvancedSearch->SearchValue)) {
            $this->birth_year->addErrorMessage($this->birth_year->getErrorMessage(false));
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdriversrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"DriverSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        $item->Visible = true;
