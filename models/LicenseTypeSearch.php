...
class LicenseTypeSearch extends LicenseType
{
    ...
    // Page object name
    public $PageObjName = "LicenseTypeSearch";
        ...
        if ($this->isPageRequest()) {
            ...
            if ($this->isSearch()) {
                ...
                if ($srchStr != "") {
                    ...
                    $srchStr = "LicenseTypeList" . "?" . $srchStr;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->modifyUserID->EditValue = HtmlEncode($this->modifyUserID->AdvancedSearch->SearchValue);
            ...
            $curVal = trim(strval(UnFormatDateTime($this->modifyWhen->AdvancedSearch->SearchValue, $this->modifyWhen->formatPattern())));
            if ($curVal != "") {
                $this->modifyWhen->AdvancedSearch->ViewValue = $this->modifyWhen->lookupCacheOption($curVal);
            } else {
                $this->modifyWhen->AdvancedSearch->ViewValue = $this->modifyWhen->Lookup !== null && is_array($this->modifyWhen->lookupOptions()) ? $curVal : null;
            }
            if ($this->modifyWhen->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->modifyWhen->EditValue = array_values($this->modifyWhen->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", UnFormatDateTime($this->modifyWhen->AdvancedSearch->SearchValue, $this->modifyWhen->formatPattern()), DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->modifyWhen->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCacheImpl($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->modifyWhen->EditValue = $arwrk;
            }
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->modifyUserID->AdvancedSearch->SearchValue)) {
            $this->modifyUserID->addErrorMessage($this->modifyUserID->getErrorMessage(false));
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("LicenseTypeList"), "", $this->TableVar, true);
        $pageId = "search";
