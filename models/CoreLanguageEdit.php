 ...
 * Page class
 */
class CoreLanguageEdit extends CoreLanguage
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'core_language';

    // Page object name
    public $PageObjName = "CoreLanguageEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (core_language)
        if (!isset($GLOBALS["core_language"]) || get_class($GLOBALS["core_language"]) == PROJECT_NAMESPACE . "core_language") {
            $GLOBALS["core_language"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'core_language');
        }
    ...
    protected function getRecordsFromRecordset($rs, $current = false)
    {
        ...
        if (is_object($rs)) { // Recordset
            while ($rs && !$rs->EOF) {
		        ...
		        $this->imageURL->OldUploadPath = "../img/lang/";
		        $this->imageURL->UploadPath = $this->imageURL->OldUploadPath;
    ...
    public function run()
    {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("CoreLanguageList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("CoreLanguageList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "CoreLanguageList") {
    ...
    protected function getUploadFiles()
    {
        ...
        $this->imageURL->Upload->Index = $CurrentForm->Index;
        $this->imageURL->Upload->uploadFile();
        $this->imageURL->CurrentValue = $this->imageURL->Upload->FileName;
    }
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->imageURL->Upload->DbValue = $row['imageURL'];
        $this->imageURL->setDbValue($this->imageURL->Upload->DbValue);
    ...
    protected function newRow()
    {
        ...
        $row['imageURL'] = $this->imageURL->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        // imageURL
        $this->imageURL->RowCssClass = $this->IsMobileOrModal ? "row" : "";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // imageURL
            $this->imageURL->UploadPath = "../img/lang/";
            if (!EmptyValue($this->imageURL->Upload->DbValue)) {
                $this->imageURL->ImageAlt = $this->imageURL->alt();
                $this->imageURL->ImageCssClass = "ew-image";
                $this->imageURL->ViewValue = $this->imageURL->Upload->DbValue;
            } else {
                $this->imageURL->ViewValue = "";
            }
            $this->imageURL->ViewCustomAttributes = "";
            ...
            // imageURL
            $this->imageURL->LinkCustomAttributes = "";
            $this->imageURL->UploadPath = "../img/lang/";
            if (!EmptyValue($this->imageURL->Upload->DbValue)) {
                $this->imageURL->HrefValue = GetFileUploadUrl($this->imageURL, $this->imageURL->htmlDecode($this->imageURL->Upload->DbValue)); // Add prefix/suffix
                $this->imageURL->LinkAttrs["target"] = ""; // Add target
                if ($this->isExport()) {
                    $this->imageURL->HrefValue = FullUrl($this->imageURL->HrefValue, "href");
                }
            } else {
                $this->imageURL->HrefValue = "";
            }
            $this->imageURL->ExportHrefValue = $this->imageURL->UploadPath . $this->imageURL->Upload->DbValue;
            ...
            // imageURL
            $this->imageURL->setupEditAttributes();
            $this->imageURL->EditCustomAttributes = "";
            $this->imageURL->UploadPath = "../img/lang/";
            if (!EmptyValue($this->imageURL->Upload->DbValue)) {
                $this->imageURL->ImageAlt = $this->imageURL->alt();
                $this->imageURL->ImageCssClass = "ew-image";
                $this->imageURL->EditValue = $this->imageURL->Upload->DbValue;
            } else {
                $this->imageURL->EditValue = "";
            }
            if (!EmptyValue($this->imageURL->CurrentValue)) {
                $this->imageURL->Upload->FileName = $this->imageURL->CurrentValue;
            }
            if ($this->isShow()) {
                RenderUploadField($this->imageURL);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->imageURL->Required) {
            if ($this->imageURL->Upload->FileName == "" && !$this->imageURL->Upload->KeepFile) {
                $this->imageURL->addErrorMessage(str_replace("%s", $this->imageURL->caption(), $this->imageURL->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // imageURL
        if ($this->imageURL->Visible && !$this->imageURL->ReadOnly && !$this->imageURL->Upload->KeepFile) {
            $this->imageURL->Upload->DbValue = $rsold['imageURL']; // Get original value
            if ($this->imageURL->Upload->FileName == "") {
                $rsnew['imageURL'] = null;
            } else {
                $rsnew['imageURL'] = $this->imageURL->Upload->FileName;
            }
        }
        ...
        if ($this->imageURL->Visible && !$this->imageURL->Upload->KeepFile) {
            $this->imageURL->UploadPath = "../img/lang/";
            $oldFiles = EmptyValue($this->imageURL->Upload->DbValue) ? [] : [$this->imageURL->htmlDecode($this->imageURL->Upload->DbValue)];
            if (!EmptyValue($this->imageURL->Upload->FileName)) {
                $newFiles = [$this->imageURL->Upload->FileName];
                $NewFileCount = count($newFiles);
                for ($i = 0; $i < $NewFileCount; $i++) {
                    if ($newFiles[$i] != "") {
                        $file = $newFiles[$i];
                        $tempPath = UploadTempPath($this->imageURL, $this->imageURL->Upload->Index);
                        if (file_exists($tempPath . $file)) {
                            ...
                            $file1 = UniqueFilename($this->imageURL->physicalUploadPath(), $file); // Get new file name
                            if ($file1 != $file) { // Rename temp file
                                while (file_exists($tempPath . $file1) || file_exists($this->imageURL->physicalUploadPath() . $file1)) { // Make sure no file name clash
                                    $file1 = UniqueFilename([$this->imageURL->physicalUploadPath(), $tempPath], $file1, true); // Use indexed name
                                }
                ...
                $this->imageURL->Upload->DbValue = empty($oldFiles) ? "" : implode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $oldFiles);
                $this->imageURL->Upload->FileName = implode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $newFiles);
                $this->imageURL->setDbValueDef($rsnew, $this->imageURL->Upload->FileName, null, $this->imageURL->ReadOnly);
            }
        }
        ...
        if ($updateRow) {
            ...
            if ($editRow) {
                if ($this->imageURL->Visible && !$this->imageURL->Upload->KeepFile) {
                    ...
                    if (!EmptyValue($this->imageURL->Upload->FileName)) {
                        $newFiles = [$this->imageURL->Upload->FileName];
                        $newFiles2 = [$this->imageURL->htmlDecode($rsnew['imageURL'])];
                        $newFileCount = count($newFiles);
                        for ($i = 0; $i < $newFileCount; $i++) {
                            if ($newFiles[$i] != "") {
                                $file = UploadTempPath($this->imageURL, $this->imageURL->Upload->Index) . $newFiles[$i];
                                if (file_exists($file)) {
                                    if (@$newFiles2[$i] != "") { // Use correct file name
                                        $newFiles[$i] = $newFiles2[$i];
                                    }
                                    if (!$this->imageURL->Upload->SaveToFile($newFiles[$i], true, $i)) { // Just replace
                    ...
                    if (Config("DELETE_UPLOADED_FILES")) {
                        foreach ($oldFiles as $oldFile) {
                            if ($oldFile != "" && !in_array($oldFile, $newFiles)) {
                                @unlink($this->imageURL->oldPhysicalUploadPath() . $oldFile);
                            }
                        }
                    }
                }
        ...
        if ($editRow) {
            // imageURL
            CleanUploadTempPath($this->imageURL, $this->imageURL->Upload->Index);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("CoreLanguageList"), "", $this->TableVar, true);
        $pageId = "edit";
