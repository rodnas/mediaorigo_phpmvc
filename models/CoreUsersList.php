 ...
 * Page class
 */
class CoreUsersList extends CoreUsers
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CoreUsersList";
    ...
    public $FormName = "fcore_userslist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "CoreUsersAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "CoreUsersDelete";
        $this->MultiUpdateUrl = "CoreUsersUpdate";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->core_groupsID->AdvancedSearch->toJson(), ","); // Field core_groupsID
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcore_userssrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field core_groupsID
        $this->core_groupsID->AdvancedSearch->SearchValue = @$filter["x_core_groupsID"];
        $this->core_groupsID->AdvancedSearch->SearchOperator = @$filter["z_core_groupsID"];
        $this->core_groupsID->AdvancedSearch->SearchCondition = @$filter["v_core_groupsID"];
        $this->core_groupsID->AdvancedSearch->SearchValue2 = @$filter["y_core_groupsID"];
        $this->core_groupsID->AdvancedSearch->SearchOperator2 = @$filter["w_core_groupsID"];
        $this->core_groupsID->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->core_groupsID, $default, false); // core_groupsID
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->core_groupsID->AdvancedSearch->save(); // core_groupsID
        }
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->core_groupsID->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->core_groupsID->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->core_groupsID->AdvancedSearch->load();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->core_groupsID); // core_groupsID
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->lastLoginWhen->setSort("");
                $this->activationCode->setSort("");
                $this->regmailWhen->setSort("");
                $this->activationWhen->setSort("");
                $this->core_groupsID->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcore_userslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcore_userslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("nickname", $this->createColumnOption("nickname"));
            ...
            $option->add("core_groupsID", $this->createColumnOption("core_groupsID"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcore_userssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcore_userssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcore_userslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // nickname
        if ($this->nickname->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->nickname->AdvancedSearch->SearchValue != "" || $this->nickname->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // core_groupsID
        if ($this->core_groupsID->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->core_groupsID->AdvancedSearch->SearchValue != "" || $this->core_groupsID->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (!$this->nickname->Raw) {
                $this->nickname->AdvancedSearch->SearchValue = HtmlDecode($this->nickname->AdvancedSearch->SearchValue);
            }
            ...
            if (!$Security->canAdmin()) { // System admin
                ...
                $curVal = trim(strval($this->core_groupsID->AdvancedSearch->SearchValue));
                if ($curVal != "") {
                    $this->core_groupsID->AdvancedSearch->ViewValue = $this->core_groupsID->lookupCacheOption($curVal);
                } else {
                    $this->core_groupsID->AdvancedSearch->ViewValue = $this->core_groupsID->Lookup !== null && is_array($this->core_groupsID->lookupOptions()) ? $curVal : null;
                }
                if ($this->core_groupsID->AdvancedSearch->ViewValue !== null) { // Load from cache
                    $this->core_groupsID->EditValue = array_values($this->core_groupsID->lookupOptions());
                } else { // Lookup from database
                    if ($curVal == "") {
                        $filterWrk = "0=1";
                    } else {
                        $filterWrk = "`id`" . SearchString("=", $this->core_groupsID->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                    }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcore_userssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"CoreUsersSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        $item->Visible = true;

        // Search highlight button
        $item = &$this->SearchOptions->add("searchhighlight");
        $item->Body = "<a class=\"btn btn-default ew-highlight active\" role=\"button\" title=\"" . $Language->phrase("Highlight") . "\" data-caption=\"" . $Language->phrase("Highlight") . "\" data-ew-action=\"highlight\" data-form=\"fcore_userssrch\" data-name=\"" . $this->highlightName() . "\">" . $Language->phrase("HighlightBtn") . "</a>";
