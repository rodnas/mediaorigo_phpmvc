...
class Cargo extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->place->InputTextType = "text";
        $this->place->Nullable = false; // NOT NULL field
        $this->place->Required = true; // Required field
        $this->Fields['place'] = &$this->place;

        // cargo_weight
        $this->cargo_weight = new DbField(
            'cargo',
            'cargo',
            'x_cargo_weight',
            'cargo_weight',
        ...
        $this->cargo_weight->InputTextType = "text";
        $this->cargo_weight->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['cargo_weight'] = &$this->cargo_weight;
        ...
        $this->transport_date = new DbField(
            ...
            'transport_date',
            '`transport_date`',
            CastDateFieldForLike("`transport_date`", 0, "DB"),
            133,
            10,
            0,
            false,
        ...
        $this->transport_date->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->place->DbValue = $row['place'];
        $this->cargo_weight->DbValue = $row['cargo_weight'];
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "CargoEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "CargoAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("CargoView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("CargoView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "CargoAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "CargoAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("CargoEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CargoAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("CargoDelete", $this->getUrlParm());
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->place->EditValue = $this->place->CurrentValue;
        ...
        $this->cargo_weight->EditValue = $this->cargo_weight->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->place);
                    $doc->exportCaption($this->cargo_weight);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->place);
                        $doc->exportField($this->cargo_weight);
