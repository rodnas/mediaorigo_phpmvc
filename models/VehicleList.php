 ...
 * Page class
 */
class VehicleList extends Vehicle
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VehicleList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "VehicleAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "VehicleDelete";
        $this->MultiUpdateUrl = "VehicleUpdate";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->vehicle_typeID->AdvancedSearch->toJson(), ","); // Field vehicle_typeID
        $filterList = Concat($filterList, $this->lpn->AdvancedSearch->toJson(), ","); // Field lpn
        $filterList = Concat($filterList, $this->start_year->AdvancedSearch->toJson(), ","); // Field start_year
        $filterList = Concat($filterList, $this->person->AdvancedSearch->toJson(), ","); // Field person
        $filterList = Concat($filterList, $this->max_weight->AdvancedSearch->toJson(), ","); // Field max_weight
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fvehiclesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field vehicle_typeID
        $this->vehicle_typeID->AdvancedSearch->SearchValue = @$filter["x_vehicle_typeID"];
        $this->vehicle_typeID->AdvancedSearch->SearchOperator = @$filter["z_vehicle_typeID"];
        $this->vehicle_typeID->AdvancedSearch->SearchCondition = @$filter["v_vehicle_typeID"];
        $this->vehicle_typeID->AdvancedSearch->SearchValue2 = @$filter["y_vehicle_typeID"];
        $this->vehicle_typeID->AdvancedSearch->SearchOperator2 = @$filter["w_vehicle_typeID"];
        $this->vehicle_typeID->AdvancedSearch->save();

        // Field lpn
        $this->lpn->AdvancedSearch->SearchValue = @$filter["x_lpn"];
        $this->lpn->AdvancedSearch->SearchOperator = @$filter["z_lpn"];
        $this->lpn->AdvancedSearch->SearchCondition = @$filter["v_lpn"];
        $this->lpn->AdvancedSearch->SearchValue2 = @$filter["y_lpn"];
        $this->lpn->AdvancedSearch->SearchOperator2 = @$filter["w_lpn"];
        $this->lpn->AdvancedSearch->save();
        ...
        // Field person
        $this->person->AdvancedSearch->SearchValue = @$filter["x_person"];
        $this->person->AdvancedSearch->SearchOperator = @$filter["z_person"];
        $this->person->AdvancedSearch->SearchCondition = @$filter["v_person"];
        $this->person->AdvancedSearch->SearchValue2 = @$filter["y_person"];
        $this->person->AdvancedSearch->SearchOperator2 = @$filter["w_person"];
        $this->person->AdvancedSearch->save();

        // Field max_weight
        $this->max_weight->AdvancedSearch->SearchValue = @$filter["x_max_weight"];
        $this->max_weight->AdvancedSearch->SearchOperator = @$filter["z_max_weight"];
        $this->max_weight->AdvancedSearch->SearchCondition = @$filter["v_max_weight"];
        $this->max_weight->AdvancedSearch->SearchValue2 = @$filter["y_max_weight"];
        $this->max_weight->AdvancedSearch->SearchOperator2 = @$filter["w_max_weight"];
        $this->max_weight->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->vehicle_typeID, $default, false); // vehicle_typeID
        $this->buildSearchSql($where, $this->lpn, $default, false); // lpn
        $this->buildSearchSql($where, $this->start_year, $default, false); // start_year
        $this->buildSearchSql($where, $this->person, $default, false); // person
        $this->buildSearchSql($where, $this->max_weight, $default, false); // max_weight
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->vehicle_typeID->AdvancedSearch->save(); // vehicle_typeID
            $this->lpn->AdvancedSearch->save(); // lpn
            $this->start_year->AdvancedSearch->save(); // start_year
            $this->person->AdvancedSearch->save(); // person
            $this->max_weight->AdvancedSearch->save(); // max_weight
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        $searchFlds = [];
        $searchFlds[] = &$this->id;
        $searchFlds[] = &$this->vehicle_typeID;
        $searchFlds[] = &$this->lpn;
        $searchFlds[] = &$this->start_year;
        $searchFlds[] = &$this->person;
        $searchFlds[] = &$this->max_weight;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->vehicle_typeID->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->lpn->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->start_year->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->person->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->max_weight->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->vehicle_typeID->AdvancedSearch->unsetSession();
        $this->lpn->AdvancedSearch->unsetSession();
        $this->start_year->AdvancedSearch->unsetSession();
        $this->person->AdvancedSearch->unsetSession();
        $this->max_weight->AdvancedSearch->unsetSession();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->vehicle_typeID); // vehicle_typeID
            $this->updateSort($this->lpn); // lpn
            $this->updateSort($this->start_year); // start_year
            $this->updateSort($this->person); // person
            $this->updateSort($this->max_weight); // max_weight
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->setSessionOrderBy($orderBy);
                $this->id->setSort("");
                $this->vehicle_typeID->setSort("");
                $this->lpn->setSort("");
                $this->start_year->setSort("");
                $this->person->setSort("");
                $this->max_weight->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fvehiclelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fvehiclelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("vehicle_typeID", $this->createColumnOption("vehicle_typeID"));
            $option->add("lpn", $this->createColumnOption("lpn"));
            $option->add("start_year", $this->createColumnOption("start_year"));
            $option->add("person", $this->createColumnOption("person"));
            $option->add("max_weight", $this->createColumnOption("max_weight"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fvehiclesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fvehiclesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fvehiclelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        if ($this->vehicle_typeID->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->vehicle_typeID->AdvancedSearch->SearchValue != "" || $this->vehicle_typeID->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->lpn->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->lpn->AdvancedSearch->SearchValue != "" || $this->lpn->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->person->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->person->AdvancedSearch->SearchValue != "" || $this->person->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->max_weight->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->max_weight->AdvancedSearch->SearchValue != "" || $this->max_weight->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fvehiclesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"VehicleSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        $item->Visible = true;
