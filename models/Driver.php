...
/**
 * Table class for driver
 */
class Driver extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'driver';
        $this->TableName = 'driver';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`driver`";
        $this->Dbid = 'DB';
        ...
        $this->birth_year->InputTextType = "text";
        $this->birth_year->Required = true; // Required field
        $this->birth_year->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['birth_year'] = &$this->birth_year;

        // license_typeID
        $this->license_typeID = new DbField(
            'driver',
            'driver',
            'x_license_typeID',
            'license_typeID',
        ...
        $this->license_typeID->InputTextType = "text";
        $this->license_typeID->Nullable = false; // NOT NULL field
        $this->license_typeID->Required = true; // Required field
        $this->license_typeID->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->license_typeID->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        switch ($CurrentLanguage) {
            case "en-US":
                $this->license_typeID->Lookup = new Lookup('license_typeID', 'license_type', false, 'id', ["name","","",""], [], [], [], [], [], [], '', '', "`name`");
                break;
            default:
                $this->license_typeID->Lookup = new Lookup('license_typeID', 'license_type', false, 'id', ["name","","",""], [], [], [], [], [], [], '', '', "`name`");
                break;
        }
        $this->license_typeID->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['license_typeID'] = &$this->license_typeID;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->modifyUserID->Lookup = new Lookup('modifyUserID', 'passanger', false, 'id', ["customer_name","how_many_people","transport_date",""], [], [], [], [], [], [], '', '', "CONCAT(COALESCE(`customer_name`, ''),'" . ValueSeparator(1, $this->modifyUserID) . "',COALESCE(`how_many_people`,''),'" . ValueSeparator(2, $this->modifyUserID) . "',COALESCE(" . CastDateFieldForLike("`transport_date`", "y-MM-dd", "DB") . ",''))");
                break;
            default:
                $this->modifyUserID->Lookup = new Lookup('modifyUserID', 'passanger', false, 'id', ["customer_name","how_many_people","transport_date",""], [], [], [], [], [], [], '', '', "CONCAT(COALESCE(`customer_name`, ''),'" . ValueSeparator(1, $this->modifyUserID) . "',COALESCE(`how_many_people`,''),'" . ValueSeparator(2, $this->modifyUserID) . "',COALESCE(" . CastDateFieldForLike("`transport_date`", "y-MM-dd", "DB") . ",''))");
                break;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`driver`";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->birth_year->DbValue = $row['birth_year'];
        $this->license_typeID->DbValue = $row['license_typeID'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("DriverList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "DriverEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "DriverAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("DriverView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("DriverView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "DriverAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "DriverAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("DriverEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("DriverAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("DriverDelete", $this->getUrlParm());
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->birth_year->EditValue = $this->birth_year->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->birth_year);
                    $doc->exportCaption($this->license_typeID);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->birth_year);
                        $doc->exportField($this->license_typeID);
