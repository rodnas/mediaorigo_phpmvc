 ...
 * Page class
 */
class CoreGroupsAdd extends CoreGroups
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CoreGroupsAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("CoreGroupsList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "CoreGroupsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "CoreGroupsView") {
    ...
    protected function addRow($rsold = null)
    {
        ...
        } elseif ((int)$this->id->CurrentValue < -2) {
            $this->setFailureMessage($Language->phrase("UserLevelIDIncorrect"));
        } elseif ((int)$this->id->CurrentValue == 0 && !SameText($this->name->CurrentValue, "Default")) {
            $this->setFailureMessage($Language->phrase("UserLevelDefaultName"));
        } elseif ((int)$this->id->CurrentValue == -1 && !SameText($this->name->CurrentValue, "Administrator")) {
            $this->setFailureMessage($Language->phrase("UserLevelAdministratorName"));
        } elseif ((int)$this->id->CurrentValue == -2 && !SameText($this->name->CurrentValue, "Anonymous")) {
            $this->setFailureMessage($Language->phrase("UserLevelAnonymousName"));
        } elseif ((int)$this->id->CurrentValue > 0 && in_array(strtolower(trim($this->name->CurrentValue)), ["anonymous", "administrator", "default"])) {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("CoreGroupsList"), "", $this->TableVar, true);
