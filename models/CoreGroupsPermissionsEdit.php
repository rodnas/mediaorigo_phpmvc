 ...
 * Page class
 */
class CoreGroupsPermissionsEdit extends CoreGroupsPermissions
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CoreGroupsPermissionsEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("core_groupsID") ?? Key(0) ?? Route(2)) !== null) {
                $this->core_groupsID->setQueryStringValue($keyValue);
                $this->core_groupsID->setOldValue($this->core_groupsID->QueryStringValue);
            } elseif (Post("core_groupsID") !== null) {
                $this->core_groupsID->setFormValue(Post("core_groupsID"));
                $this->core_groupsID->setOldValue($this->core_groupsID->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("core_groupsID") ?? Route("core_groupsID")) !== null) {
                    $this->core_groupsID->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->core_groupsID->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("CoreGroupsPermissionsList"); // Return to list page
                        return;
                        ...
                        if ($this->core_groupsID->CurrentValue != null && $this->_tablename->CurrentValue != null) {
                            while (!$rs->EOF) {
                                if (SameString($this->core_groupsID->CurrentValue, $rs->fields['core_groupsID']) && SameString($this->_tablename->CurrentValue, $rs->fields['tablename'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("CoreGroupsPermissionsList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "CoreGroupsPermissionsList") {
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_core_groupsID")) {
            $this->core_groupsID->setOldValue($CurrentForm->getValue("o_core_groupsID"));
        }
    ...
    public function renderRow()
    {
        ...
        // tablename
        $this->_tablename->RowCssClass = $this->IsMobileOrModal ? "row" : "";

        // permission
        $this->_permission->RowCssClass = $this->IsMobileOrModal ? "row" : "";
    ...
    protected function editRow()
    {
        ...
        // core_groupsID
        $this->core_groupsID->setDbValueDef($rsnew, $this->core_groupsID->CurrentValue, 0, $this->core_groupsID->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("CoreGroupsPermissionsList"), "", $this->TableVar, true);
        $pageId = "edit";
